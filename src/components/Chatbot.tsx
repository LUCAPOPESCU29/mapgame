...
function getChatSystemPrompt(think: boolean, deepSearch: boolean): string {
  if (think && deepSearch) {
    return `You are Europa Temporis — an expert historian and scholar AI.
...
You may use markdown-style formatting (bold, lists) for clarity.`;
  ...
  if (think) {
    return `You are Europa Temporis — a thoughtful historical guide.
...
Give analytical, well-reasoned historical responses.`;
  ...
  if (deepSearch) {
    return `You are Europa Temporis — a comprehensive historical researcher.
...
Be thorough and specific.`;
  ...
  return `You are Europa Temporis — a knowledgeable and engaging historical guide.
